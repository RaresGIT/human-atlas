...
const Row=memo(function Row({atlas,concept,side,selected,hidden,onSelect,onFocus}:Omit<Props,'concepts'>&{concept:Concept}){
 ...
 return <div className="structure-row"><div className="structure-row-main"><Button variant="ghost" aria-pressed={selected?.id===concept.id&&selected.elements.length===elements.length} onClick={()=>onSelect(chosen)} onDoubleClick={()=>onFocus(chosen)}>{concept.name}<span>{elements.every(id=>hidden.includes(id))?'Hidden':`${elements.length} pieces`}</span></Button>{elements.length>1&&<Button variant="ghost" aria-label={`${expanded?'Collapse':'Expand'} ${concept.name}`} aria-expanded={expanded} onClick={()=>setExpanded(v=>!v)}>{expanded?'−':'+'}</Button>}</div>
 {expanded&&<div className="structure-members">{elements.slice(0,limit).map(id=>{const p=index.parts.get(id);if(!p)return null;const c={id:p.conceptId,name:p.name,elements:[id]};return <Button key={id} variant="ghost" aria-pressed={selected?.elements.length===1&&selected.elements[0]===id} onClick={()=>onSelect(c)} onDoubleClick={()=>onFocus(c)}>{p.name}{hidden.includes(id)&&<span>Hidden</span>}</Button>;})}{limit<elements.length&&<Button variant="outline" onClick={()=>setLimit(n=>n+30)}>Show next 30 pieces</Button>}</div>}
 </div>;
});
