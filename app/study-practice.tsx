...
export default function StudyPractice({quiz,count,ready,sourceName,onStart,onAnswer,onNext,onEnd}:Props){
  ...
  if(!quiz)return <section aria-label="Practice setup"><h2>Identification practice</h2><p className="study-note">{sourceName} · Practice {count} named structures. Each concept can contain several modeled pieces.</p><div className="study-practice-modes"><Button disabled={!count||!ready} onClick={()=>onStart('name')}>Name a structure</Button><p>Type the atlas name of the highlighted structure. Case, spacing, and punctuation are ignored.</p><Button disabled={!count||!ready} onClick={()=>onStart('find')}>Find a structure</Button><p>Click or tap any piece belonging to the requested structure in a separated set. Pan and zoom to inspect small pieces.</p></div>{!ready&&<p role="status">Wait for the anatomy to finish loading.</p>}</section>;
  ...
  return <section aria-label="Identification question">
    <p className="study-note">{sourceName} · {quiz.mode==='name'?'Name':'Find'} · Question {quiz.index+1} of {quiz.questions.length} · {quiz.correct} correct</p>
